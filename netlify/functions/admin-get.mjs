...
export default async (req) => {
  ...
  const [state, requests, ratings] = await Promise.all([
    getState(),
    readAll("salta-requests"),
    readAll("salta-ratings")
  ]);

  let changed = false;
  for (const p of state.professionals || []) {
    if (!p.portalToken) {
      p.portalToken = crypto.randomUUID().replaceAll("-", "") + crypto.randomUUID().replaceAll("-", "");
      changed = true;
    }
  }
